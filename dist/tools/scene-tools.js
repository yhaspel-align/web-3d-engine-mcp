...
export function registerSceneTools(server, relay) {
    server.registerTool('get_scene_objects', {
        description: 'Get all rendered 3D objects in the current scene, organized as a tree. ' +
    ...
    server.registerTool('get_cameras', {
        description: 'Get all cameras in the scene. Returns an array with name, uuid, cameraPosition, and groupRelated for each camera.',
        inputSchema: z.object({}),
    }, async () => {
    ...
    server.registerTool('get_materials', {
        description: 'Get material properties for all rendered objects. Returns shader uniforms, vertex/fragment shaders, and material types per object UUID.',
        inputSchema: z.object({}),
    }, async () => {
    ...
    server.registerTool('get_object_transformation', {
        description: 'Get the current transformation matrix (Matrix4) for an object by UUID. ' +
