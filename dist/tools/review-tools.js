...
export function registerReviewTools(server, relay) {
    server.registerTool('select_image', {
        description: 'Select a NIRI image by ID via pluginsSyncStore.setImageSelection. ' +
        ...
        if (rotationDegrees !== undefined)
            args.push(rotationDegrees);
        if (rotationByQuarters !== undefined)
            args.push(rotationByQuarters);
    ...
    server.registerTool('move_loupe', {
    ...
    server.registerTool('highlight_loupe', {
