...
export function registerPluginTools(server, relay) {
    server.registerTool('set_jaw_navigation', {
        description: 'Toggle jaws on or off in the JawsNavigation plugin, exactly mimicking a user click on the jaw selector. ' +
        ...
        const overrides = {};
        if (upper_jaw !== undefined)
            overrides['JawsNavigation'] = { ...(overrides['JawsNavigation'] ?? {}), upper_jaw: { cameraPosition, selected: upper_jaw } };
        if (lower_jaw !== undefined)
            overrides['JawsNavigation'] = { ...(overrides['JawsNavigation'] ?? {}), lower_jaw: { cameraPosition, selected: lower_jaw } };
        if (Object.keys(overrides).length === 0)
            return errResponse('Provide at least one of upper_jaw or lower_jaw');
        const r = await relay.execute('refresh_engine_context', { overrides });
        if (r.error)
            return errResponse(r.error);
        const parts = [];
        if (upper_jaw !== undefined)
            parts.push(`upper_jaw: ${upper_jaw}`);
        if (lower_jaw !== undefined)
            parts.push(`lower_jaw: ${lower_jaw}`);
        return { content: [{ type: 'text', text: `JawsNavigation updated — ${parts.join(', ')}` }] };
    });
    server.registerTool('set_plugin_props', {
        description: 'Merge props into any plugin preset via the Angular generateEngineContext(overrides) path. ' +
    ...
    server.registerTool('set_plugin_active', {
    ...
    server.registerTool('set_plugin_disabled', {
        description: 'Force-disable a plugin via UPDATE_PLUGIN_PRESET with isForcedDisabled. ' +
    ...
    server.registerTool('set_plugin_zone', {
        ...
        const idx = presets.findIndex((p) => (p?.component)?.id === componentId);
        if (idx === -1)
            return errResponse(`Preset with component.id "${componentId}" not found`);
        presets[idx] = { ...presets[idx], ...zoneData };
    ...
    server.registerTool('add_or_update_plugin_preset', {
    ...
    server.registerTool('remove_plugin_preset', {
        ...
        if (ctx.error)
            return errResponse(ctx.error);
        const current = ctx.result;
        const presets = (current.preset ?? []).filter((p) => (p?.component)?.id !== componentId);
        const r = await relay.execute('update_engine_context', { data: { preset: presets } });
        if (r.error)
            return errResponse(r.error);
        return { content: [{ type: 'text', text: `Plugin preset "${componentId}" removed` }] };
    });
    server.registerTool('set_stage_splitter_view', {
        description: 'Update the StageSplitter plugin preset with activeView and optional cameraPositions/ignoreModels.',
        inputSchema: z.object({
        ...
        if (cameraPositions !== undefined)
            props.cameraPositions = cameraPositions;
        if (ignoreModels !== undefined)
            props.ignoreModels = ignoreModels;
    ...
    server.registerTool('set_active_jaw', {
        description: 'Set the active jaw in the jaw selector. ' +
