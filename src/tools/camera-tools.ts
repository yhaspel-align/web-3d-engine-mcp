...
export function registerCameraTools(server: McpServer, relay: WebSocketRelay): void {
  server.registerTool(
    'focus_camera_to_object',
    {
      description:
        'Focus the camera on a specific object in the scene. ' +
  ...
  server.registerTool(
    ...
    {
      description:
        'Rotate the camera by a specified number of degrees around an axis (x, y, or z). ' +
  ...
  server.registerTool(
    ...
    {
      description:
        'Move the camera to an explicit position in 3D space. ' +
  ...
  server.registerTool(
    'toggle_camera_static',
    {
      description:
        'Enable or disable static mode on a camera. In static mode the camera cannot be moved by user interaction.',
      inputSchema: z.object({
