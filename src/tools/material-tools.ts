...
export function registerMaterialTools(server: McpServer, relay: WebSocketRelay): void {
  server.registerTool(
    'change_object_color',
    {
      description:
        'Change the uniform (solid) color of an object. ' +
  ...
  server.registerTool(
    ...
    {
      description:
        'Change material properties on an object such as opacity, wireframe mode, or other shader uniforms. ' +
        'Identify the object by name or UUID.',
      inputSchema: z.object({
        properties: z
          .record(z.unknown())
          .describe(
            'Key-value map of material properties to update. ' +
