...
export function registerContextTools(server: McpServer, relay: WebSocketRelay): void {
  server.registerTool(
    'set_feature_flag',
    {
      description:
        'Set a single feature flag in engineContext.featuresAvailability. ' +
    ...
    async ({ flag, value }) => {
      ...
      const features = { ...(current.featuresAvailability as Record<string, unknown> ?? {}), [flag]: value };
  ...
  server.registerTool(
    'update_engine_context_partial',
    {
      description:
        'Partial update of engine context for approved paths only (UI, bizCtx, featuresAvailability). ' +
    ...
    async ({ data }) => {
      ...
      for (const key of Object.keys(data)) {
        if (BLOCKED.includes(key)) return errResponse(`Key "${key}" is not allowed for MCP updates`);
      }
