...
export function registerCameraViewTools(server: McpServer, relay: WebSocketRelay): void {
  server.registerTool(
    'set_stage_camera_positions',
    {
      description:
        'Set StageSplitter camera positions preset. Uses predefined positions like Frontal, Left-Buccal, Right-Buccal, Upper-Occlusal, Lower-Occlusal.',
    ...
    async ({ cameraPositions, activeView }) => {
      const data: Record<string, unknown> = { cameraPositions };
  ...
  server.registerTool(
    'set_render_view_camera_props',
    {
      description:
        'Update camera props for a specific render view cell. The bridge reconstructs Vector3 instances for position/up.',
      inputSchema: z.object({
  ...
  server.registerTool(
    'set_render_view_fit_padding',
    {
      description:
        'Update the modelPaddingFactor on current render views. This is the stateful zoom/fit control. ' +
    ...
    async ({ modelPaddingFactor }) => {
      ...
      if (views.error) return errResponse(views.error);
      const rvp = views.result as Record<string, unknown>;
      if (!rvp) return errResponse('No render view props available');
      (rvp as Record<string, unknown>).modelPaddingFactor = modelPaddingFactor;
      const r = await relay.execute('call_store_action', {
        store: 'rendererStore',
  ...
  server.registerTool(
    ...
    async ({ enabled }) => {
      ...
      const features = { ...(current.featuresAvailability as Record<string, unknown> ?? {}), isRotationEnabled: enabled };
  ...
  server.registerTool(
    'rotate_camera_to_jaw_view',
    {
      description:
        'Directly rotate the Three.js camera to a named view position for a specific jaw. ' +
  ...
  server.registerTool(
    ...
    {
      description:
        'Show or hide a jaw mesh in the 3D view by setting jawsVisibility on all render views. ' +
    ...
    async ({ jaw, visible }) => {
      ...
      const rvp = views.result as Record<string, unknown>[][];
      if (!rvp || !Array.isArray(rvp)) return errResponse('No render view props available');

      const jawKey = `${jaw}Jaw` as string;
      for (const row of rvp) {
        for (const cell of row) {
          const jv = cell.jawsVisibility as Record<string, unknown>;
          if (jv) jv[jawKey] = visible;
        }
      }
